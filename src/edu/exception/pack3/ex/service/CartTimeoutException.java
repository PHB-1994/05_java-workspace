 ...
 * 강제 중단 상황에 따른 대처를 서비스에서 진행한다.
 * 
 * ex) 티켓 예매 사이트 에서
 *              동일한 아이디가 다른 주소로 3개 접속되어 있는 것을 발견
 *              동일한 아이디 동시접속 Exception extends Exception
 *             
 *              --> 동일한 아이디가 여러 곳에서 티케팅을 하려고 접속한 것을 확인했습니다.
 *                  모두 로그아웃 처리를 진행합니다. 와 같이 설정해줄 수 있음
 * 결제처리 = 결제시간만료 / 다른 사람이 먼저 결제했고, 상품이 품절됐을 때 결제기능 중단 처리
 * 쿠폰적용
