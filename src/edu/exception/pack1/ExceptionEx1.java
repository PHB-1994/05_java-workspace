...
public class ExceptionEx1 {
    ...
    public void method1(){
        ...
        // 프로그램 실행 중에 이와 같이 예외 상황이 발생하는 경우
        // 예외 상황에 맞는 코드가 수행되며
        // 자바 개발자가 만들어 놓은 예외 출력문을 던진 후 실행 종료됨
        
        // 자바 개발자가 만든 폴더 위치와 폴더 내 존재하는 파일
        //                  java.lang.ArrayIndexOutOfBoundsException
        //                배열과 숫자 관련해서 문제가 발생하면 Index 숫자 out of bounds for length 숫자
        //                로 어떤 상황인지 개발자한테 문제되는 내용을 전달할 수 있도록 만들어 놓음
    }
    ...
    - catch(예외){} : try 구문 시도 중 예외가 발생되어 던져진 경우
                        catch(예외)에 작성된 예외와 던져진 예외와 같으면
                        잡아서 {} 내 구문을 수행
    ...
    public void method2(){
        ...
        try{ // 문제가 발생할 것 같은 코드들을 넣음
            for(int i = 0; i <= arr.length; i++){
                System.out.println(arr[i]);
            }
        } catch(ArrayIndexOutOfBoundsException e){
            // try 에서 작성한 코드들 중에
            // 개발자가 예상했을 때 배열에서 index 번호 보다
            // 더 큰 자리값을 요구할 때 발생하는 문제가 생길 것같으면
