...
public class ArrayEx1 {
    /* 배열 (자료구조)
        ...
        참조형 변수란 ? -> 개발자가 만들어놓은 클래스 파일을 참조하는 변수 이름
     */
    ...
    public void method1(){
        // 변수 선언
        int num;
        // static 영역에 int 자료형을 저장할 수 있는 변수를 생성하고
        // 그 변수의 이름을 "num"으로 지정

        // 배열 선언
        int[] arr;
        // static 영역에 int[] 자료형을 참조할 변수를 생성하고 (주소 저장)
        // 그 변수의 이름을 "arr"으로 지정
        ...
        // -> arr 를 이용해서 배열을 참조해서 데이터를 사용할 수 있음

        // 배열 초기화

        // 1) 초기화 전 -> 0 이 대입되어 있는지 확인
        System.out.println("초기화 전 배열의 상태");
        System.out.println("arr 가 위치한 컴퓨터 내의 주소 명칭 : " + arr);
        System.out.println("arr 의 초기값의 길이 : " + arr.length);
        ...
        arr 의 초기값 : [I@2f4d3709
        arr 의 초기값의 길이 : 4
        Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: Index 4 out of bounds for length 4
	    at edu.array.ArrayEx1.method1(ArrayEx1.java:64)
	    at edu.array.ArrayRun.main(ArrayRun.java:12)
	    
	    Index 4 out = index 배열이 배치될 수 있는 칸은 4칸인데 4칸 이상을 작성해서 발생하는 예외상황
        자바스크립트에서는 length 의 길이보다 데이터가 추가되면 자연스럽게 length 의 길이도 커지지만
        자바는 최로로 지정한 칸 수를 그대로 유지하며, 칸 수를 초과할 경우 예외상황 발생
        
        추후 ArrayList 로 자바스크립트 처럼 최초로 지정한 칸 수에 관계없이 데이터를 추가할 수 있는
        class 파일을 자바 개발자가 만든 후 자바 버전을 업그레이드하였고,
        개발자들은 ArrayList 와 같은 기능을 활용해서 유연하게 데이터를 추가하고 삭제할 수 있음
         */
    ...
    public void method2(){
        ...
        // index 는 length 의 -1 한 값
        // index 0 번 부터 length -1 번까지만 추가할 것
