...
public class Korean {
    ...
    public static int nationalCode = 82; // 우리나라 국가 코드
    ...
    /**
     * main 메서드에서
     * Korean k1 = new Korean(); // static 에 작성된 nationalCode 는 여기서 최초 1회 실행
     * Korean k2 = new Korean(); // static 으로 작성된 nationalCode 는 다시 실행되지 않음
     */

    /* **********
     * 일반 초기화 블럭 : user 가 접속할 때 마다 guest 부여할 때
     * k1, k2, k3 와 같은 유저마다 guest 칭호를 붙이고
     * k1 이 admin 이면 k1 만 추후 admin 으로 교체
    ...
    // 반환형 String : 기능 실행 후 종료 시 String 에 해당하는 데이터를 전달
     ...
     *      보통 return; 작성은 method 의 최하위에 작성하는 것이 아니라
     *
    ...
    public int method2(){
        int b = 1;
        return b; // 1 대신에 1 이라는 값을 담고있는 b 변수이름을 대신 활용
    }

    public int method3(int c){
        // 나중에 매개변수 자리로 들어오는 숫자값을
        // 활용해서 추후 c 로 반환
        // c 에 더하기 .. 특정 기능을 추가적으로 작성한 후 반환할 수 있음
        return c;
    }
