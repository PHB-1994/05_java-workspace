package edu.oop.cls.constructor;
...
@기본생성자          세터를 이용해서 데이터를 하나씩 하나씩 html 에서 가져온 데이터를 담아놓는 것
@매개변수존재생성자   세터를 이용하지 않고 데이터를 한 번에 html 에서 가져온 데이터를 담아놓는 것
@게터                DB 에 저장된 데이터를 자바에서 가져와 갖고 있는 상태
@세터                DB 에 저장하기 전에 자바에서 소비자가 작성한 이름, 이메일과 같은 데이터를 가지고 있는 상태
 */
public class Member {
    ...
    // 객체를 생성하고, 객체를 프로그램이 실행하는 동안 지속적으로 사용하기 위하여
    // 객체에 특정 명칭을 붙여 활용
    ...
    // 호출하여 m 이라는 이름으로 객체로 불러온 문서(= 사람이 작성한 코드 설계도면)을 활용
    ...
    // 매개변수 생성자가 존재하고 기본 생성자가 존재하지 않는 경우 매개변수 생성자만 존재하고
    // 기본 생성자를 생성하지 않기 때문에 매개변수 생성자가 존재할 때는 기본 생성자 또한 작성
    ...
    ***************** MemberRun class 파일 *****************
    ...
    //    추후 main 이나 service 에서 호출하여 자바 프로그램 실행 시
    //    객체 형태로 만들어진 생성자
    //              외부에서 전달받은 데이터를 갖고 있는 변수이름  변수 = 데이터가 들어있는 공간의 명칭
    public Member(String memberId, String memberName, String memberPw, int memberAge) {
        // 현재 Member 클래스에서
        // private 이나 public
        // default protected          = 외부에서 new Member(매개변수); 형태로 데이터랄 가지고 온
        // 형태로 작성된 변수 이름     = 데이터가 들어있는 변수 이름
        // this.변수이름              = 매개변수명칭
        this.memberId                 = memberId;
        this.memberName               = memberName;
        this.memberPw                 = memberPw;
        this.memberAge                = memberAge;
        //                              html 에서 가져온 데이터를 담고 있는 변수 이름과
        // Member 클래스에서 작성한
        // 변수 이름과 일치하는 공간
        //                              매개변수에서 가져온 데이터를
        // this. 변수이름에 담아서 DB 에 저장하겠다.
    }

    // 매개 변수 타입이 다름
    // 다른건 몰라도 memberName 은 무조건 갖고 있는 상태에서 나를 활용할 것
