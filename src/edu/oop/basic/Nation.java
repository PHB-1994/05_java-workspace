...
// 클래스 : 객체가 가져야 할 속성(값), 기능(메서드)를 글(코드) 형태로 작성해둔 문서
//         문서를 추후에 new 클래스이름() 생성자를 이용하여 객체로 호출
/**
 * 국민 객체를 만들기 위한 클래스
 */
public class Nation {
    // private 대신 public 을 이용하여 다른 클래스 파일에서 Nation 에 작성된 속성 기능 호출해서 사용
    ...
    /* *****************
        Getter 를 활용하는 경우
         ...
         4. 계산 작업할 때
         ------------------------------
         현재는 getter 를 활용해서 setter 에 저장된 데이터를 출력하지만
         DB 에 저장된 값을 전달하는 용도
         회원가입 시 이메일 인증번호 일치 여부와 같이 특정 데이터에 대한 값 확인
         주로 프론트엔드로 DB 에 존재하는 데이터 전달할 때 많이 사용
         
         수정하기 -> DB 에서 수정해야하는 게시물 번호 get 가져오기
                 -> 게시물 번호에 해당하는 데이터를 get 으로 전달하여 프론트엔드에서 JS 로 전달받아
    ...
    /** 메서드 생성자 기본 필수 작성
     *        필수 생성자의 경우 필드 속성값에 작성되어있는 변수명칭 순서대로 작성
     */

    // 메서드 setter(데이터 간접적으로 저장하는) getter(데이터를 간접적으로 사용하는) 완성
    ...
    /**
     * 자기소개 기능
     * private 작성한 클래스 내부에서만 변수이름을 사용 가능하게 끔 설정
     * name,age,gender 는 외부에서 변수이름을 직접적으로 호출해서 사용 불가
     * public void introduce() 메서드 내부에서 변수이름을 호출하고
     * 외부에서 name 대신에 introduce() 기능을 이용해서 변수이름에 해당하는 값 확인 가능
     */
    public void introduce(){
        System.out.printf("이름은 %s 이거, %d세 %c성 입니다. \n",name,age,gender);
    }
}
