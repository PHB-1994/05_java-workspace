...
public class AccountRun {
    // JVM 바라보는 메인 메서드      args 의 매개변수 이름은 변경해도 되지만 나머지는 그대로 유지
    // 되도록이면 수정 없이 사용!
    public static void main(String[] args){
        ...
        a1.setName("홍길동"); // name 이라는 변수이름을 간접적으로 사용하는 setName 에서
        // 홍길동 이라는 명칭을 가지고 있는 상태

        // 간접 접근 기능을 이용해서 객체 내부에 저장되어 있는 데이터를 출력
        a1.getName(); // setName 으로 저장한 데이터를 가져오기
        // getName 은 가져오는 것이지 가져오는 것을 출력하는 메서드는 아님!!!
        // getName 으로 가져온 데이터를 확인하기 위해서는 System 과 같은 출력문 내부에 작성해야함
        System.out.println(a1.getName());
        a1.setAccountNumber("123-456-789");
        a1.setPassword("7890");

        /*
        Account class 에서 accountNumber 를 String
        자료형으로 작성한다 표기했기 때문에
        ...
        // 매개변수를 작성한 공간으로 Account() 소괄호 내부에 작성한 매개변수 순서대로 데이터를 작성해야함
        // 데이터 순서가 일치하지 않으면 에러 발생
        Account a2 = new Account("강말숙","111-222-3333",10000,"7777");

        System.out.println("===== a2 의 계좌 정보 =====");
