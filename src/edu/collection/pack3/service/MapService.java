...
public class MapService {
    ...
    public void method1(){
        ...
        //                      인덱스 아님. 순서없음 9번부터 작성해도 됨. 번호 이어서 작성 안해도 됨
        //                      개발자가 필요로 하는 공간의 명칭
        ...
        System.out.println(map1.put(3,"스테이크앤치즈")); // map 에서 put 으로 null 이 나오는 건 데이터가 없다는 의미
                                                         // 3 이라는 키 명칭에 들어있는 데이터가 없습니다. (데이터 들어오는 것을 환영합니다.)
        ...
        // list 목록은 대괄호 형태로 출력됨 [  ] 배열 모양


        // 2. V get(K k) : Key 에 해당되는 Value 반환
        //                                  일치하는 Key 가 없으면 null 반환
        System.out.println("-".repeat(20));
        System.out.println(map1.get(1)); // 에그마요
        System.out.println(map1.get(0)); // null 의 데이터를 반환

        String tamp1 = map1.get(2); // map1.get(2) 를 직접적으로 데이터 확인하는 것이 아니라
        System.out.println(tamp1);  // tamp1 이라는 공간에 담은 후 담겨있는 데이터를 확인
