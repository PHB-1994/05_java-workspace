...
public class SetService {
    ...
    // - 중복 데이터 저장 불가 (같은 데이터가 들어오면 덮어쓰기 됨)
    ...
    public void method1(){
        // 사용법 : List  ArrayList 와 동일하고, List 에서 Set 으로 변경된 것 일 뿐!
        ...
        // 다음에 접근할 데이터가 존재하나요? while 문 주로 사용
        // List 는 인덱스 번호가 존재하지만 Set 의 경우에는 데이터의 순번이나 키 명칭이 존재하지 않기 때문
    ...
    boolean containers(E e) : 요소가 존재하면 true 없으면 false

    void      clear(  ) : Set 에 저장된 내용 모두 삭제
    set.clear()          모두 제거
    ...
    public void method2(){
        ...
        과자들.add("빈츠");
        과자들.remove("꼬북칩");
        System.out.println("과자들에서 빈츠거 잘 지워졌나요~? : " + 과자들.remove("빈츠"));

        과자들.clear(); // 과자들에 있는 과자들 모~두 삭제
    ...
    public void lotto(){
        ...
        while(lotto.size() < 6){
            // random.nextInt(45)   0 < x < 45 사이의 알 수 없는 수를 생성
