...
public class ListService {
    ...
    public void method1(){
        ...
        // - 대표적으로 ArrayList(클래스 문서가 존재) List 의 후손 클래스
        ...
        System.out.println("list의 1번 째 index : " + list.get(1));
        
        // 3. int size()       : List 에 저장된 요소(객체)의 개수를 반환. length 대신 사용
        System.out.println("list 변수이름에 저장된 데이터의 개수 : " + list.size());
        
        // List 에서 ArrayList 는 언제 사용하는가?
        // -> 회원 목록 조회, 검색한 데이터를 모두 가져올 때, 상품리스트 모두 조회 등
        // 데이터를 DB 에서 2개 이상 가지고 온 후, 사용자에게 보여주어야 할 때 사용
    }
    ...
    public void method2(){
        ...
        //  ↓ <>  필수로 String 형태만 담을 수 있도록 설정   ↓ <> 에 String 을 작성해도 되고, 안해도 됨
        ...
        //                  strList 에서 필요한 타입은 String 인데 int 타입이 제공되어서 발생하는 문제
        ...
        System.out.println("=".repeat(20) + " for 문을 활용하여 출력하기 " + "=".repeat(20));
        // get 처럼 반복 형태의 경우 for 문을 활용해서 한번에 출력가능
        for(int i = 0; i < strList.size(); i++){
            System.out.println("strList" + i + "번 째 : " + strList.get(i));
        }

        // 3. void add(int index, E e) : index 번 째 중간에 삽입 가능
        strList.add(1,"막국수");
        ...
        // index 번 째 요소를 e로 수정
        // -> 수정 전 요소가 반환 된 다음 수정됨
        // 수정전str 변수이름에는 치킨에서 양념치킨으로 데이터가 수정되기 전의 데이터 상태로 저장됨
        // 그리고 나서 strList 2번 째 index 의 데이터가 변화됨
        ...
        // 6. int indexOf(E e) :
        // List 내부에 e 와 같은 요소가 있으면 해당 인덱스 값을 반환, 없으면 무조건 -1 값을 반환
        ...
        // List 내부에 e 와 같은 요소가 있으면 true, 없으면 false. 요소 존재 유무
...
""");
            ...
            input = sc.nextInt();
            sc.nextLine(); // 스캐너에 남아있는 개행 문자 제거 (자동으로 한줄 바꿈 버퍼 생성)
            ...
            switch(input){
                ...
                case 1:
                    ...
                    // List 는 int String boolean 이외 모~~든 것을 담을 수 있음
                ...
                case 2:
                    ...
                    if(bookList.size() > 0){
                        ...
                        // for 문 다형성에서 작성했던 것처럼 향상된 for 문 활용하기
                        for(Book book : bookList) System.out.println(book);
                    } else System.out.println("존재하는 도서가 존재하지 않습니다.");
                    break;
