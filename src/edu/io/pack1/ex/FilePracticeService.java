...
public class FilePracticeService {
    ...
    **예상 출력 예시:**
    ...
    public void method2(){
        // java_basic 폴더에서 src/edu/oop/method/ex 까지 경로 설정
...
1. 절대 경로 `C:/io_test/temp` 폴더 내부에 `delete_target.txt` 라는 이름의 파일을 생성합니다.
    (단, `C:/io_test/temp` 폴더가 존재하지 않을 수 있으니 파일 생성 전에 폴더가 있는지 확인하고,
    없다면 `mkdirs()`로 먼저 생성해야 합니다.)
...
4. 파일 삭제 후, 다시 `exists()` 메소드를 호출하여 파일이 정말로 삭제되었는지(false가 반환되는지) 확인하고,
     ...
     */
    public void method3(){
        String path = "/io_test/temp";
        String fName = "delete_target.txt";

        // 1. 절대 경로로 폴더와 파일을 참조하는 File 객체 생성
        File tempDir = new File(path);
        File f = new File(tempDir + "/" + fName);
        // /io_test/temp/delete_target.txt
        ...
        if(!tempDir.exists()){
            ...
            System.out.println("이미 생성된 폴더의 명칭입니다.");
        }
        try {
            // 파일 생성 시도
            f.createNewFile();
