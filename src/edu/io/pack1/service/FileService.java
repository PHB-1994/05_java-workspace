...
/*******************⭐⭐⭐⭐⭐ 개발자 상식 ⭐⭐⭐⭐⭐*******************
 OS : 운영 체제
    ...
    단순슬래시(/, Linux, Mac -> C:/폴더이름1/폴더이름2/....

    Java 언어의 특징 : 플랫폼(OS)의 독립적으로 윈도우, 리눅스, 맥북에 관계없이
                      똑같은 코드를 작성 가능한 것처럼
                      \, / 둘 다 호환이 가능
...
public class FileService {
    ...
    boolean mkdirs()         : 경로상에 존재하는 모든 디렉토리 생성(폴더 여러 개 한 번에 생성 make directory(=folder))
    ...
    File[]  listFiles()      : 디렉토리 내 파일 목록일 File[] 배열로 반환
     */
    ...
    public void method2(){
        ...
        try { // 컴퓨터에서 코드를 실행할 때 발생할 수 있는 문제 코드를 실행/시도할 것이다.
            파일하나.createNewFile();
        } catch (IOException e) { // 실행/시도할 때 문제가 생성되면
            // 예외 상황으로 인하여 자바 전체 프로그램을 중지하는 것이 아니라
            // 특정 기능에 대한 문제를 개발자가 회사나 프로그램에 알맞는 방향으로
            // 기능이 진행될 수 있도록 설정
            System.out.println("파일을 생성하는 도중 문제가 발생했습니다.");
        }catch(Exception e){
            System.out.println("개발자가 발견하지 못한 문제들을 확인. 추후 문제들에 대하여");
            ...
            // e.getMessage(); e.printStackTrace(); 둘 중 하나 택해서 사용하면 됨!
        }
    ...
    public void method3(){
        ...
        try {
            ...
            if(!폴더만드는객체.exists()){ // 개발자가 원하는 해당 폴더 경로가 없는게 사실이라면
                폴더만드는객체.mkdir();
    ...
    public void method4(){
        ...
        // /io_test/practice/filesmethod4.txt / /io_test/practice폴더 내 filesmethod4.txt 생성하는 것
        ...
        */
        ...
        if(!fPath.exists()){ // /io_test/practice/files 경로가 존재하지 않는다면
            fPath.mkdir();  // 회사와 개발자가 원하는 경로 폴더들을 생성하기 기능 실행
            System.out.println(fPath.getName() + " 폴더을 생성합니다.");
        }

        try {
            fFile.createNewFile(); // 폴더를 만드는 도중 문제가 발생할 수 있으니 try catch 로 감싸서 실행
            // 그래야지 프로그램이 중간에 멈추지 않음
            System.out.println(fFile.getName() + " 파일이 생성되었습니다.");

        } catch (IOException e) {
            System.out.println(fPath + "폴더가 존재하지 않습니다. 폴더 먼저 생성해주세요.");
    ...
    /*
    자바 개발자들이 만든 File 클래스의 기능들 활용하기
