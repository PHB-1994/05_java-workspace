...
public class FileEx {
    ...
    파일 정보 및 관리
    File             : 파일이나 폴더의 존재 여무, 정보 확인하는 레거시 클래스
    ...
    * 유틸리티(Utility) : 편리한 기능들을 모아놓은 클래스
    ...
    왜 레거시라고 부르는가 ?
    오래되어서 문제가 많지만 기존 코드와 호환성 때문에 아직 남아있음
    새 프로젝트에서는 잘 사용하진 않음
     */
    public void legacyMethod(){
        ...
        if(!success){
            // 삭제를 못한게 사실이라면
            // 왜 삭제를 못했는가 ? 권환이 없어서 ? 다른 프로그램에서 사용중인지 ?
    ...
    public void FactoryMethod(){
        ...
        // 윈도우와 맥북 or 리눅스 환경을 구분해야 해서 좋은 방식이 아님
        ...
        //                User\\user\\file.text 와 샅은 형태의 슬래시가 알아서 생성
        ...
        // Paths 와 Path 구분하지 못한 채 사용하는 경우가 많아 Path 라는 명칭으로 통일해서
    ...
    /*
        ...
        Path.getNameCount()   = 파일 위치에 도달하기까지 몇 개의 / 를 거치는가 ? 경로 개수 세는 것
        ...
        Files.move(Path.of("원본파일경로/원본파일이름.확장자이름"),Path.of("이동해놓을파일경로/(선택적으로변경)파일이름.확장자이름"),StandardCopyOption.REPLACE_EXISTING);
                                                                                                                                 파일 이동할 때 덮어쓰기 형태로 파일이동
                                                                                                                                 
        Files.delete(Path.of("경로/파일이름.확장자이름"));         -> 파일 삭제하는데 삭제 못하면 예외 발생
        ...
        Streamer = 인터넷을 통해 데이터의 연속적인 흐름을 이용해서 영상이나 음성 데이터를 실시간으로 흘려보내는 사람

        Viewer   = 스트리머의 방송을 보는 사람들

        시정차가 스트리머의 방송을 보던 중 갑자기 화면이 정지되더니 버퍼링 중입니다 !!!!!

        버퍼링중입니다 = 데이터를 가져오는 중입니다.
