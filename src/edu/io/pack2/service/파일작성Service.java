...
public class 파일작성Service {
    ...
    /*
    Files.writeString() : Java 11 부터 도입된 기능으로
                          간단히 문자열을 파일에 쓰는 방법

    파일을 작성할 때
    파일을 열고, 파일 내부에 글자를 작성한다 표기하고,
    try-catch 를 이용해서 파일 생성하는데 문제가 없는지 확인하면서 진행
    영어, 숫자 이외 글자를 컴퓨터에서 글자 깨짐 없이 작성될 수 있도록
    ...
    Paths   : Path 기능들을 활용해서 만든 문서 파일
                -> 이 두가지를 사용하지 않아도 경로를 Files.getPath() 와 같은 기능으로 파일의 경로를 가져올 수 있지만...
                    ...
                    File 문서는 파일처리만 진행하자..
                    Paths 문서를 이용해서 경로에 해당하는 건 이 문서를 통해서 이용하자
                    역할에 따른 명확한 분리가 필요하다 여겨 Paths 가 생성된 것이지만
                    Files.getPath() 를 이용해도 큰 문제는 없다..
                    
                    마치 Math.random() 이 존재하고 Random.random() 존재하는 것과 같은 개념
                    
                    수학 - 랜덤 기능              랜덤자체를 분리해서 사용


                   Files            .            writeString     (Paths.get(fileName)     ,      파일내용);
    파일2버전 문서를 활용해서 내부에 존재하는   문자열작성기능사용   (문서가위치하는경로       , 문서에 들어갈 내용)
     */
    public void method1(){
...
//        File fFilename = new File(path + "/" + fileName);
        ...
        try {
            ...
            System.out.println("파일 생성 및 컨텐츠작성이 완료되었습니다.");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
