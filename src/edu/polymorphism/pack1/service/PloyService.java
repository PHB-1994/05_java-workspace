...
public class PloyService {
    ...
    // 스마트폰 전용 집  집이름  = 각 공간에 iphone 이 살고, galaxy 가 살고 있는 것
         SmartPhone     s1 = new Iphone();
         SmartPhone     s2 = new Galaxy();
    // 부모가 만든 공간에서              자식이 사는 것 가능
    // 자식이 만든 공간에서              본인이 사는 것 가능
    
    // 자식이 만든 공간에서              부모가 사는 것 불가능 => 자식이 공간이 더 크기 때문에
    
    int 인트변수 = (int)100L; // int 의 공간은 숫자이므로 숫자를 int 로 강제 형변환 해서 가능
    // 가능한 이유는 둘다 숫자임을 자바에서 알고 있기 때문에 가능

    // SmartPhone 과 Iphone, Galaxy 는 자바가 아는 형태가 아니라 개발자가 만들어놓은 형태이기 때문에 형변환 어려움
    //     ?상자      ?상자   ?상자
    // Iphone iphone3 = (Iphone) new SmartPhone(); // 마찬가지로 형변환해서 사용하면 가능하나 데이터가 변경되며 손실이 일어날 수 있음
    ...
    //	at edu.polymorphism.pack1.service.PloyService.<init>(PloyService.java:27)
    // 현재 코드에서는 문제가 일어나지 않지만 코드 실행 하면 ClassCastException 클래스가 일치하지 않습니다.
    // 하며 데이터 변환 시 데이터가 손실되어 문제가 발생
    // SmartPhone 을 상속받는 Galaxy 와 같은 데이터가 잘못 들어올 수 있기 때문에
    ...
    public void method1(){
        ...
        // s1.iosVersion(20); -> SmartPhone 공간이기 때문에 Iphone 작성된 필드는 사용 못함
    ...
    public void method2(){
        ...
        // 스마트폰 4대에 대한 정보를 sps 라는 공간에 한 번에 넣어줄 수 있다.
        스마트폰공장[0] = 스마트폰1대; // 각각의 스마트폰 공장에 스마트폰 1대의 대한 정보를 저장
        스마트폰공장[1] = 스마트폰2대;
        스마트폰공장[2] = 스마트폰3대;

        스마트폰공장[0].setDisplay("아이폰 14  디스플레이"); // 와 같은 형식으로 작성 가능
        스마트폰공장[1].setDisplay("갤럭시 S23 디스플레이"); // 와 같은 형식으로 작성 가능
        스마트폰공장[2].setDisplay("파이폰     디스플레이"); // 와 같은 형식으로 작성 가능
        /*
        SmartPhone[] 스마트폰공장 =  new SmartPhone[4];
        // 스마트폰 4대에 대한 정보를 sps 라는 공간에 한 번에 넣어줄 수 있다.
        스마트폰공장[0] = 스마트폰1대; // 각각의 스마트폰 공장에 스마트폰 1대의 대한 정보를 저장
        스마트폰공장[1] = 스마트폰2대;
        스마트폰공장[1] = 스마트폰3대; // 스마트폰[2]에 대한 객체 정보가 존재하지 않아 발생되는 에러

        스마트폰공장[0].setDisplay("아이폰 14  디스플레이"); // 와 같은 형식으로 작성 가능
        스마트폰공장[1].setDisplay("갤럭시 S23 디스플레이"); // 와 같은 형식으로 작성 가능
        스마트폰공장[2].setDisplay("파이폰     디스플레이"); // 와 같은 형식으로 작성 가능

        Exception in thread "main" java.lang.NullPointerException: Cannot invoke "edu.polymorphism.pack1.model.SmartPhone.setDisplay(String)" because "스마트폰공장[2]" is null
	    at edu.polymorphism.pack1.service.PloyService.method2(PloyService.java:64)
         */

        // 스마트폰 공장 내에 존재하는 스마트폰들의 정보를 for 문을 활용해서 모두 출력
        for(SmartPhone    기기확인 : 스마트폰공장){
            System.out.println("공장 내 기기들의 디스플레이 확인 : " + 기기확인.getDisplay());
        }

        // 반복적으로 출력했던 기능들을 for 문을 활용해서 한 번에 출력 가능
    }
}
