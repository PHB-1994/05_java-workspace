...
/**
 * Meta플랫폼을 추상 클래스로 변경 후
 * Facebook 과 Instagram 에서 implements 가 없이
 * post() 기능과 like() 기능을 사용할 수 있도록 추상 메서드로 설정
 */
public abstract class Meta플랫폼 {
    ...
    // 메서드 생성자 = main 을 실행했을 대 데이터를 담을 수 있도록 공간을 생성해주는 메서드
    ...
    // getter setter. getter : 클라이언트나 DB 에서 잔달 받은 데이터를 사용할 수 있도록 하는 기능
    //                setter : 클라이언트한테 전달받은 데이터를 프로그램이 실행되는 동안 임시로 저장하여 보유하고 있는 기능
    ...
    public void login(){
        System.out.println(get유저아이디() + " 님이 " + get플랫폼이름() + " 에 로그인 했습니다.");
    }
    public void logout(){
        System.out.println(get유저아이디() + " 님이 " + get플랫폼이름() + " 에 로그아웃 했습니다.");
    }
