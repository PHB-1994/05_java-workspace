...
public class VariableEx1 {
    public static void main(String[] args) {
        ...
        [문자형]
        char    (2byte)     - 문자 하나, 문자 양 옆에 '' 작 은 따 옴 표 사용
        ...
        */
        ...
        isTrue = false;
        System.out.println("isTrue : " + isTrue); // false 로 덮어쓰기 되는 것을 확인할 수 있음

        // 변수선언 + 초기화 + 값의 범위 확인
        int number2 = 2100000000; // 21억 오류 X
        // 숫자자료형들은 리터럴값이 기본적으로 int 이기 때문
        // byte short int long 은 모두 값(=리터럴)에서 뒤에 자료형 명칭이 붙지 않는 한 모두 int
        
        // long number3 = 2200000000; // int 22억 오류 integer 숫자가 너무 큽니다
        //            리터럴값을 int -> long 의 값으로 변환
        ...
        * int c = method명칭();       // 리터럴이 아닌 메서드 리턴값
        * int d = Scanner.nextInt(); // 리터럴이 아닌 사용자의 입력값
        ...
        *             그런데 nt2 에 2배 가량이 넘는 long 데이터를 담아 넣으려 하기 때문에
        *             바구니 크기에 비하여 데이터가 크다는 문제가 발생
        *
        *       값을 억지로 넣을 수 는 있지만 값이 왜곡되어 변수이름 바구니에 담김
        *       => 강제 형변환
        ...
        * */
        ...
        int nt1 = 100;
        int nt2 = (int)1000000000L; // long 데이터가 int 로 강제 형변환 데이터 변질

        // Long lng1 = 200;  -> 오류 발생
        long lng1 = 200;            // int 리터럴을 2배 가량의 메모리를 차지하는 Long 에 담을 수 있음
                                    // 큰 바구니에 작은 데이터를 담는 것은 문제가 되지 않음
                                    // long 과 같이 리터럴에 L 을 작성하는 경우에는
                                    // 자료형 타입에도 Long 작성하는 사소한 문제를 발생할 수 있음
        long lng2 = 2000000000L;    // 기본 자료형은 String 이외 모두 소문자
    }
}
