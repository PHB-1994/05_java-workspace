...
public class VariableEx3 {
    public static void main(String[] args) {
        ...
        * */
        ...
        int num1 = 290;
        // byte - 128 ~ 127 0 포함
        byte result1 = (byte)num1; // 강제로 형변환 처리
        ...
        System.out.println("result2 : " + result2); // 정수로 강제 형변화하여 소수점 아래 숫자 제거

        /* 난수 + 강제 형변환 */
        // 난수 : Math.random();
        // -> 0.0 <= x < 1.0
        // 1 ~ 10 사이의 난수
        int random = (int)(Math.random() * 10 + 1); // 0 ~ 9 까지 숫자 생성 + 1 을 해줌으로 1 ~ 10 으로 변경

        // 강제 형변환 확인 3
        // 문자(char) <-> 유니코드(int)
        System.out.println("=== 컴퓨터 상에서 각 문자의 숫자 번호 ===");
        System.out.println("A의 번호 : " + (int)'A'); // 문자 A 를 숫자로 변환하여 몇 번째 숫자 자리인지 확인
        System.out.println("a의 번호 : " + (int)'a'); // 문자 a 를 숫자로 변환하여 몇 번째 숫자 자리인지 확인
        System.out.println("가의 번호 : " + (int)'가'); // 문자 가 를 숫자로 변환하여 몇 번째 숫자 자리인지 확인
